...
Lossless audio is not sitting on a CDN: `convert_wav` renders it on demand and `wav_file/` hands back a short-lived signed S3 URL — the only path to a WAV. The public `media_urls` are an encrypted opus stream for the web player. `download.mjs` streams each WAV straight to disk and verifies its size against the clip's duration, because a truncated transfer keeps a valid `RIFF` header and only shows up as a short file.
