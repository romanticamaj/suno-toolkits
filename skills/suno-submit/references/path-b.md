# suno-submit — Path B: the model-driven loop, and the UI knowledge behind the script

**Fallback and repair reference.** `scripts/submit.mjs` is the default and encodes every gotcha
below. Read this file only when:

- a prompt sets `audio_reference` (the script refuses those — the Browse/Remix modal is not automated),
- the script broke on a Suno UI change and the batch is urgent,
- you are repairing `submit.mjs` from a `_submit_failure_*.json` dump (the **UI notes & gotchas**
  section at the end is the hard-won selector knowledge the script implements),
- the user explicitly asks to watch it step by step.

Everything below is the original procedure, kept verbatim.
