 ...
 *
 * Lives at the repo root, not inside either skill: both import it as `../../../lib/session.mjs`.
 * It used to sit under suno-submit/scripts/lib/, which made suno-download depend on another
 * skill's directory layout — copy or relocate one skill and the import broke. Shared
 * infrastructure belongs beside the skills, not inside one of them.
 *
...
export async function ensureLoggedIn(page, { profileDir, loginOnly = false, log = console.log } = {}) {
  ...
  await page.goto('https://suno.com/create', { waitUntil: 'domcontentloaded' });
  await sleep(4000);
  // skipDiagnostics: a sign-in that did not happen is not UI drift, and a caller that dumps
  // selector diagnostics on failure would otherwise litter the user's cwd with a .png and .json.
  if (!(await isLoggedIn(page))) {
    throw Object.assign(new Error('still not signed in to Suno — re-run once logged in'), { skipDiagnostics: true });
  }
  log('✓ signed in');
}
